...
# ============================================================
# FIVETRAN / RAW DATA WAIT
# ============================================================
# The sensor waits until ALL 12 RAW tables have received a
# Fivetran sync within the last hour.
#
# If data is not ready, Airflow waits 2 minutes and checks again.
# timeout=3600 means it waits up to 1 hour.
# mode="reschedule" releases the worker slot while waiting.
#
# IMPORTANT:
# This is a freshness gate, not a direct Fivetran API status check.
# It uses the _FIVETRAN_SYNCED metadata available in every RAW table.
# ============================================================
...
with DAG(
    ...
    wait_for_fivetran = SqlSensor(
        task_id="wait_for_fivetran",
        conn_id=SNOWFLAKE_CONN_ID,
        sql=FIVETRAN_READY_SQL,
        success=lambda result: result and result[0][0] == 1,
        poke_interval=120,
        timeout=3600,
        mode="reschedule",
    )
    ...
    # ========================================================
    # TASK GROUP: RAW -> STAGING
    # ========================================================
    with TaskGroup(group_id="raw_to_staging") as raw_to_staging:
    ...
    # ========================================================
    with TaskGroup(group_id="staging_to_dimensions") as staging_to_dimensions:
    ...
    # ========================================================
    with TaskGroup(group_id="fact_validation") as fact_validation:
    ...
    # ========================================================
    with TaskGroup(group_id="mart_validation") as mart_validation:
    ...
    # Fivetran freshness gate -> all RAW/STAGING checks.
    start >> wait_for_fivetran

    wait_for_fivetran >> customers
    wait_for_fivetran >> products
    wait_for_fivetran >> categories
    wait_for_fivetran >> suppliers
    wait_for_fivetran >> stores
    wait_for_fivetran >> employees
    wait_for_fivetran >> orders
    wait_for_fivetran >> order_items
    wait_for_fivetran >> payments
    wait_for_fivetran >> shipments
    wait_for_fivetran >> inventory
    wait_for_fivetran >> reviews
    ...
    # All reconciliation checks must finish before completion.
